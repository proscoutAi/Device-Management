...
class Session:
    ...
    def __init__(self, camera_index: int = 0):
        ...
        # IMU health monitoring - UPDATED for new IMU manager
    ...
    def add_payload_to_batch(self,snap_time, flow_counter, gps_data,imu_data, image):
        self.batch_payload.append({
                        ...
                        "IMU": imu_data.copy() if imu_data else [],  # Handle empty IMU data
                        "image_base_64":image,
    ...
    def check_imu_health(self):
        """Check if IMU is providing data and restart if needed - UPDATED"""
        if not self.imu_manager:
            return False
            
        current_time = time.time()
        
        # Check if IMU thread is dead but IMU still shows as active
        if self.imu_manager.thread and (not self.imu_manager.thread.is_alive()) and self.imu_manager.is_active:
            print(f"{time.ctime(time.time())}:IMU thread has died while active!")
            log_system_status()
            return self.restart_imu()
        
        # Check if we're getting fresh data only when IMU should be active
        if self.imu_manager.is_active:
    ...
    def restart_imu(self):
        """Attempt to restart the IMU manager - UPDATED"""
        ...
        try:
            # Stop existing IMU manager properly
            if self.imu_manager:
                print(f"{time.ctime(time.time())}:Stopping existing IMU manager...")
                self.imu_manager.cleanup()  # Use the proper cleanup method
                self.imu_manager = None
                gc.collect()
    ...
    def run(self):
        """The main loop of the session - UPDATED"""
        ...
        log_performance = 0 #log performance once a minute
    
        while self.running:
            # UPDATED: Only collect data when IMU is active OR when IMU is not available
    ...
    def start(self) -> bool:
        ...
        #Initialize flow meter GPIO but don't start monitoring yet - IMU will control it
        if flow_meter_connected:
          setup_flow_meter()  # Just setup GPIO, don't start monitoring thread

        # initiate IMU - UPDATED for new wake-on-motion system
        try:
            self.imu_manager = IMUManager(imu_rate_per_second)
            self.imu_last_data_time = time.time()
            print(f"{time.ctime(time.time())}:IMU initialized successfully in wake-on-motion mode")
        except Exception as e:
    ...
    def end(self) -> bool:
        ...
        # Properly stop IMU manager - UPDATED
        if self.imu_manager:
            print("Stopping IMU manager...")
            self.imu_manager.cleanup()  # Use proper cleanup method
