...
class ButtonLEDManager:
    ...
    def __init__(self):
        ...
        self.is_blinking = False  # Track if LED is currently blinking
        self.current_blink_speed = None  # Track blink speed if blinking
    ...
    def turn_on(self, color: LEDColor):
        ...
        it will be turned off first. If the same LED is already on (not blinking),
        ...
        with self.blink_lock:
            ...
            # Check if same LED is already on (not blinking)
            if self.current_led == led and not self.is_blinking:
                return
    ...
    def blink(self, color: LEDColor, speed_ms: int):
        ...
        it will be turned off first. If the same LED is already blinking at the same speed,
        the operation is skipped.
        ...
        with self.blink_lock:
            ...
            # Check if same LED is already blinking at the same speed
            if (self.current_led == led and 
                self.is_blinking and 
                self.current_blink_speed == speed_ms):
                return
